...
Date: 2026-04-01
Status: Thin-v1 is complete through Milestone 9, and the active implementation phase is now v2 at Milestone 10; the rebuilt modular web bundle now covers the operator-entry surfaces, the promoted review workbench family, and the promoted detail-route family, canonical repo build and DB-backed repo verification are passing again, and Milestone 10 implementation is now in code across Slice 1 through Slice 3 while bounded browser-review and workflow-continuity evidence still needs to catch up on the separate `docs/workflows/` track
...
| Milestone 11 operator shell and navigation direction | proposed_not_started | `milestone_11_operator_shell_and_navigation_plan.md` now records the preferred post-Milestone-10 browser direction: replace the heavy persistent left rail with a lighter top-bar shell, expose all currently supported workflows through a mix of top-level pill or bubble-style destinations plus landing pages, keep clear active-state differentiation in the navigation, allow the bubble set to wrap across multiple rows during the broad-exposure phase, add a searchable route catalog or command palette for later scaling, allow later user-level hide preferences for selected bubbles if needed, evolve `/app` into a personalized workflow-centered home surface, treat `Settings` plus privileged `Admin` as role-aware secondary utility surfaces rather than primary workflow destinations, and use one coherent soft-light low-glare visual system with dark readable text instead of stark white or dark heavy full-page backgrounds. This planning slice must not begin until Milestone 10 browser-review and workflow-continuity closeout are complete. |
...
| `internal/app` transport-boundary review | planned_future_activity | A bounded future architecture review should inspect `internal/app` for places where presentation or transport logic is drifting toward business-rule ownership. The purpose is to keep the shared backend suitable for both the rich web UI and a later mobile client on the same `/api/...` seam. If the review finds drift, follow-up work should push durable business branching back into shared service contracts or `reporting` reads rather than adding browser-specific backend behavior. |
...
12. use Slice 3 from `milestone_10_slice_3_detail_and_closeout_plan.md` as the next promoted browser implementation step: rebuild the promoted detail family and retire the remaining legacy monolithic template path
13. keep bounded browser review for the rebuilt operator-entry and review routes on the separate `docs/workflows/` track unless that review first promotes one new corrective slice back into `new_app_docs/`
14. if later validation exposes product defects or missing support seams, add bounded fix plans back into `new_app_docs/` before implementation begins
15. after Milestone 10 closeout and the next round of workflow validation, run one bounded architecture review of `internal/app` specifically for transport-orchestration boundary drift, and plan narrow refactors only where business rules or durable review composition are leaking out of shared services or `reporting`

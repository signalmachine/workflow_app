...
The post-checkpoint validation work remains important, but it is now tracked separately from implementation planning. The Milestone 10 web rebuild is implemented in code across the modular operator-entry, review-workbench, and detail-route families, while browser-review and workflow-continuity evidence still remain open on the separate `docs/workflows/` track. Active implementation has now also moved through the full Milestone 11 slice set, the first six Milestone 12 maintenance slices, and the first three implementation checkpoints of Milestone 13: `web/` now contains the landed SvelteKit scaffold with a sidebar-plus-topbar shell, protected app-route and public login-route structure, shared primitives, minimal session bootstrap against `/api/session`, unit-test coverage for the frontend API client, direct build output into `internal/app/web_dist`, the migrated Svelte workflow surfaces for home, inbound request intake, operations landing and feed, coordinator chat, route discovery, review landing, and the promoted review-list families, the migrated Svelte admin route family for `/app/admin`, `/app/admin/accounting`, `/app/admin/parties`, `/app/admin/parties/<party-id>`, `/app/admin/access`, and `/app/admin/inventory`, and the promoted exact Svelte detail routes for inbound requests, approvals, proposals, documents, accounting journal entries, inventory movements, work orders, and audit events on shared `/api/review/...` detail seams. `internal/app` can now also serve that built SPA under `/app` behind `WORKFLOW_WEB_FRONTEND=svelte` while the Go-template browser remains the default reference implementation until later cutover. The earlier browser shell no longer uses the heavy persistent left rail, the top bar groups route families under calmer landing pages, `/app/operations`, `/app/review`, and `/app/inventory` act as bundle entry points on the same shared backend seam, `/app/routes` provides searchable destination-only route discovery, `/app/settings` plus access-scoped `/app/admin` establish the utility-surface posture, the Svelte shell now hides privileged maintenance navigation from non-admin actors, `/app/admin/accounting` now covers accounting setup plus bounded ledger-account and tax-code active or inactive controls plus accounting-period maintenance, `/app/admin/parties` now covers customer and vendor support-record setup plus exact-detail contact creation and party status governance on the same shared party model, `/app/admin/access` now covers bounded org-user and membership-role controls, `/app/admin/inventory` now covers bounded inventory item and location setup plus active or inactive controls, and `/app` now behaves as a role-aware operator home rather than a permanently generic dashboard. Slice 2 also adds shared `/api/navigation/...` snapshot endpoints so route-catalog, home, operations, review landing, and agent-chat composition stay server-shaped instead of being rebuilt in client-only browser code. The `internal/app` test-suite performance and reliability pass is now also landed: the shared DB-backed harness no longer reruns schema migrations on every `dbtest.Open` call inside one test process, and the disposable advisory lock is now held only during migration or reset setup instead of for the full lifetime of each DB-backed test, while per-test resets keep the suite isolated. The live-provider checkpoint itself remains healthy: Step 1 live-provider verification was restored on 2026-03-28, the Milestone 9 closeout on 2026-03-29 reconfirmed the live path, and the first post-review rerun on 2026-03-29 also passed. The OpenAI Responses loop uses provider-safe function-tool names plus stateless continuation compatible with `store: false`, focused `go test ./internal/app -run '^TestHandleWeb' -count=1` plus `go build ./cmd/... ./internal/...` passed for the earlier shell slices, and the new Milestone 13 detail-route checkpoint verification also passed through `mcp__svelte__svelte_autofixer` on the new inventory-movement detail component, `npm --prefix web run check`, `npm --prefix web run test`, `npm --prefix web run build`, `go build ./cmd/... ./internal/...`, and the canonical `set -a; source .env; set +a; GOCACHE=/tmp/go-build go test -p 1 ./cmd/... ./internal/...`.
...
During Milestone 13 Slice 1 and Slice 2 development, the built SPA can be exercised from the Go server by enabling the frontend switch:

```bash
WORKFLOW_WEB_FRONTEND=svelte go run ./cmd/app
```

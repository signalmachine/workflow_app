...
7. the legacy UUID actor-header path is retired, and queued-agent processing on `POST /api/agent/process-next-queued-inbound-request` now uses the same browser-session or bearer-session auth contract as the rest of the shared seam

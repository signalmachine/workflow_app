...
11. inbound-request detail now also links request-level AI recommendations and downstream proposals into exact proposal review, exact approval review, filtered request review, and direct inbound-request or recommendation audit lookup so the intake-review page can continue operators into downstream control decisions instead of ending at evidence inspection
12. this milestone remains in progress because the current browser layer still needs tighter operator continuity, richer drill-downs, and focused refinement across the now-landed review surfaces
13. detailed sequencing and constraints are captured in `web_application_layer_plan.md`
14. this milestone should continue as a sequence of narrow vertical slices rather than one monolithic delivery

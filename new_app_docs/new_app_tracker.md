...
| `internal/app` transport-boundary review | second_cleanup_slice_done | The bounded cleanup review now has two implemented slices in code. The first routes repeated workflow-navigation summary composition used by the role-aware home, settings, operations landing, and review landing through one shared `reporting.WorkflowNavigationSnapshot` contract. The second now routes operations landing, durable operations feed, and inventory landing composition through dedicated shared `reporting` snapshot contracts instead of leaving those web handlers to orchestrate repeated multi-read reporting fetches directly. This keeps `internal/app` closer to route, auth, response, and rendering ownership while preserving the same browser behavior. A later bounded review can still inspect dashboard recent-request or proposal composition plus agent-chat continuity for any remaining drift before later client reuse work expands. |
...
15. treat the second `internal/app` transport-boundary cleanup slice as complete in code, focused tests, and canonical docs through the new shared reporting snapshot contracts now used by operations landing or feed and inventory landing
...
17. after Milestone 10 closeout and the next round of workflow validation, continue the bounded architecture review of `internal/app` only for the remaining route-local dashboard recent-request or proposal composition and agent-chat continuity reads, and plan narrow refactors only where business rules or durable review composition are still leaking out of shared services or `reporting`

...
The post-checkpoint validation slice is now active as of 2026-03-28. Step 1 live-provider verification has been restored: the OpenAI Responses loop now uses provider-safe function-tool names plus stateless continuation compatible with `store: false`, and `set -a; source .env; set +a; go run ./cmd/verify-agent` passes again on the configured live path.

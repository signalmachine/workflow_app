...
The Go server now serves the Svelte frontend by default. Use the legacy template browser only when a bounded fallback check is required:

```bash
WORKFLOW_WEB_FRONTEND=templates go run ./cmd/app
```

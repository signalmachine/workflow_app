...
Date: 2026-03-29
Status: Milestone 9 active; slices 1 through 3 complete, but closeout verification remains incomplete and post-checkpoint validation is still paused after partial completion
...
| Milestone 9 user-testing readiness hardening | active | The milestone remains active in `milestone_9_user_testing_readiness_hardening_plan.md`, but all three planned slices are now implemented. Slice 1 auth hardening is complete: browser-session and bearer-session issuance no longer accept org slug plus email alone, and now require a password-backed credential check on the shared `identityaccess.users` record while preserving one session foundation for the browser and the later mobile client on the same `/api/...` seam. Slice 2 bounded AI capability expansion is also complete: the OpenAI-backed coordinator keeps the same bounded coordinator-plus-specialist architecture but now exposes request-scoped read-only tools for current inbound-request detail and current processed-proposal continuity, so provider execution can gather more request-relevant review context than queue summary alone without widening into write-capable autonomy or weakening tool-policy enforcement. Slice 3 shared web or API seam decomposition is now also landed: `internal/app` is split by seam into dedicated API session, inbound, review, and approval handler files plus dedicated web session or inbound and review-surface files, reducing the largest maintenance-risk concentration without changing the shared HTTP contracts or the thin-v1 web stack. `go build ./cmd/... ./internal/...` and focused `go test ./internal/app ...` coverage passed after that decomposition. The still-incomplete portion of this session is Milestone 9 closeout verification: the documented full-suite test command failed inside the sandbox only because the sandbox could not reach the configured PostgreSQL test database, and the unrestricted rerun did not produce a final completion result before the session ended. The next session should rerun that documented full-suite command outside the sandbox, record the outcome here, and only then close Milestone 9 and resume the paused live validation slice. Anything under `examples/` remains read-only reference material and is not part of the active `workflow_app` codebase. `examples/accounting-agent-app-for-reference-only/` may still be used as a practical comparison point, but it remains a proof-of-concept single-agent reference rather than the architectural target for `workflow_app`. |
...
1. finish Milestone 9 closeout verification now that all three planned slices are implemented
2. treat the post-checkpoint validation slice as paused after partial completion rather than silently incomplete
3. rerun `set -a; source .env; set +a; GOCACHE=/tmp/go-build go test -p 1 ./cmd/... ./internal/...` outside the sandbox against the configured PostgreSQL test database and record the result
4. resume the remaining canonical live workflows for draft-amend continuity, approval-producing flow continuity, and failed-provider or failed-processing visibility immediately after Milestone 9 closes
...
1. begin with the incomplete Milestone 9 closeout verification rather than widening implementation scope again
2. rerun `set -a; source .env; set +a; GOCACHE=/tmp/go-build go test -p 1 ./cmd/... ./internal/...` outside the sandbox and record whether it passes cleanly
3. when Milestone 9 closes, return to `post_checkpoint_validation_and_user_testing_plan.md` and resume the deferred live workflows in the documented order

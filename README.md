...
The thin v2 planning documents in [`new_app_docs/`](./new_app_docs) remain the canonical source for active scope, architecture guardrails, sequencing, status, and next steps. Archived milestone detail now lives under `new_app_docs/v2_archive/` and should be used only when the current task needs that historical context.

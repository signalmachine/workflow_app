...
1. continue Milestone 6 from the now-landed provider-backed coordinator, shared backend seams, live verification command, request-submission contract, and attachment transport contract by adding operator-review backend contracts
2. widen backend contracts in small slices so request submission, attachment transport, operator review, and later browser auth can sit on one web and mobile-ready foundation
...
1. extend the now-landed API contract set on top of the OpenAI-backed coordinator, shared backend seams, and live verification command with operator-review slices after the landed request-submission and attachment-transport contracts
2. widen that backend contract set in small slices for operator review without creating a second truth owner
...
5. shared backend contracts, a focused live-provider verification command, queued-request processing, request submission, and attachment transport now exist for driving the live path outside direct service calls, so the next high-value Milestone 6 gaps are operator review and browser-usable auth on top of that seam

...
1. continue Milestone 6 from the now-landed provider-backed coordinator, shared backend agent-processing seam, live verification command, and first agent-processing API contract by adding request submission and attachment transport contracts
2. widen backend contracts in small slices so request submission, attachment transport, and operator review can sit on one web and mobile-ready foundation
...
1. extend the now-landed first HTTP API contract on top of the OpenAI-backed coordinator, shared backend agent-processing seam, and live verification command with attachment transport and request-submission slices
...
5. a shared backend contract, focused live-provider verification command, and first HTTP API contract now exist for driving the live path outside direct service calls, so the next high-value Milestone 6 gaps are request submission, attachment transport, and browser-usable auth on top of that seam

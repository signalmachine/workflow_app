...
The post-checkpoint validation work remains important, but it is now tracked separately from implementation planning. The bounded pre-validation browser work is now landed in code: the web visual refresh, the dashboard-only home, the dedicated intake page, the durable operations feed, and the dedicated agent-chat surface are all implemented, repo-verified, and ready for bounded browser review. The bounded `internal/app` test-suite performance pass is now also landed: the shared DB-backed harness no longer reruns schema migrations on every `dbtest.Open` call inside one test process, while per-test resets keep the suite isolated. Workflow validation is therefore again the next active track on the separate `docs/workflows/` layer unless that review promotes one new bounded fix slice back into `new_app_docs/`. The live-provider checkpoint itself remains healthy: Step 1 live-provider verification was restored on 2026-03-28, the Milestone 9 closeout on 2026-03-29 reconfirmed the live path, and the first post-review rerun on 2026-03-29 also passed. The OpenAI Responses loop uses provider-safe function-tool names plus stateless continuation compatible with `store: false`, `set -a; source .env; set +a; GOCACHE=/tmp/go-build go test -p 1 ./cmd/... ./internal/...` passed against the configured test database at Milestone 9 closeout, and `set -a; source .env; set +a; go run ./cmd/verify-agent` returned `REQ-000001` in `processed` state with a completed coordinator run and a request-specific urgent warehouse-pump recommendation summary. The bounded repo-verification slice for the planned foundational workflows is also complete: `internal/app` integration coverage exercises the full draft save and edit -> queue -> process continuity chain, the full processed proposal -> request approval -> approval decision continuity chain across both `/api/...` and `/app/...`, and the newly landed operations-feed plus agent-chat surfaces on the shared browser seam.

This repository has completed Milestone 0 through Milestone 9 from the canonical planning set in [`new_app_docs/`](./new_app_docs), and the explicit closeout review on 2026-03-27 marked the current codebase as thin-v1 checkpoint complete after both `go build ./...` and `set -a; source .env; set +a; go test -p 1 ./...` passed cleanly. The shared control boundary now includes adopted document ownership for work orders, invoices, and payment or receipt documents plus persist-first inbound request and attachment foundations with stable `REQ-...` inbound-request references for submission acknowledgments and review. Draft requests can now be edited or hard-deleted before queueing, while queued pre-processing requests can be soft-cancelled or returned to draft for amendment and resubmission. The browser layer now promotes that parked-request lifecycle directly into `/app`: operators can save new requests as drafts, continue new draft editing, add draft attachments, queue drafts, cancel queued requests, return queued or cancelled requests to draft, and hard-delete unprocessed drafts on the same shared backend seam that also exposes matching draft-save plus queue, cancel, amend, and delete actions over `/api/inbound-requests` and `/api/inbound-requests/{request_id}/{action}`. The promoted home page is now dashboard-only, new intake has moved to dedicated `GET /app/submit-inbound-request` plus `POST /app/inbound-requests` browser surfaces, `GET /app/operations-feed` now provides a durable one-way operational event page, and `GET /app/agent-chat` now provides a separate coordinator-guidance surface that still persists onto the shared inbound-request foundation through a dedicated `agent_chat` channel. Milestone 8 was explicitly bounded to five planned client-neutral hardening slices in the canonical tracker rather than being treated as an open-ended bucket: lifecycle contract hardening, review-read contract hardening, attachment contract hardening, approval-action contract hardening, and non-browser auth-evolution planning. Those five slices are complete, and the queued-agent auth follow-up is now closed on the same shared seam: browser-session cookies remain the active thin-v1 browser auth path, `POST /api/session/token` issues device-scoped JSON session metadata plus opaque short-lived bearer access tokens and refresh tokens on the same `identityaccess.sessions` foundation, `POST /api/session/refresh` rotates refresh plus access material, `GET /api/session` plus `POST /api/session/logout` accept bearer auth as well as cookies, and `POST /api/agent/process-next-queued-inbound-request` now also requires the same cookie or bearer session auth instead of the retired UUID actor-header compatibility path. Milestone 9 is now complete: browser-session and bearer-session login require org slug, user email, and password-backed verification on the shared `identityaccess.users` record while preserving one session foundation and one shared `/api/...` seam for the browser and the later mobile client; the OpenAI-backed coordinator still runs the same hard-capped coordinator-plus-specialist flow but can now use request-scoped current-request detail and current processed-proposal continuity tools in addition to the org-level status summary tool; and `internal/app` is decomposed by seam into dedicated API session, inbound, review, and approval handler files plus dedicated web session or inbound and review-surface files without changing the shared HTTP contracts or thin-v1 web stack. Strict JSON decoding rejects malformed or unknown request bodies across session login, token refresh, queued-request processing, inbound-request submission and mutation, and approval-decision paths; inbound-request submit, draft, queue, cancel, and amend responses now return lifecycle timestamps plus cancellation or failure reasons; malformed exact-ID filters across the shared `/api/review/...` reads now fail cleanly with `invalid review filter` instead of leaking database-cast failures; attachment upload plus download behavior is now bounded more tightly through media-type validation, malformed attachment-ID rejection, and authenticated download headers including `Content-Disposition`, `Cache-Control: private, no-store`, and `X-Content-Type-Options: nosniff`; approval decisions now reject malformed approval IDs, keep body validation aligned with the other hardened JSON endpoints, and return current approval plus document state metadata on conflict responses so later non-browser clients can recover without an extra follow-up read; and processed-proposal review now derives pre-approval document and suggested-queue continuity from recommendation payload while the shared `/api/review/processed-proposals/{recommendation_id}/request-approval` plus `/app/review/proposals/{recommendation_id}/request-approval` seams can request workflow approval directly from a processed proposal with atomic recommendation-link persistence. Failed queued-request processing now also preserves exact request continuity across both promoted seams: the API failure response carries the claimed `REQ-...` reference, and the browser failure path redirects back to the exact request detail instead of a generic dashboard error. The dashboard now also acts as a stronger operator starting surface for draft, queued, processing, failed, cancelled, processed, and completed request states by sorting request summary cards into operator-priority order, replacing generic summary actions with status-specific continuation links, surfacing failure and cancellation reasons directly in recent-request rows, and tightening recent-request actions into exact draft or lifecycle or execution or proposal continuations instead of passive status review. The final Milestone 7 closeout sweep also fixed the last browser continuity gap by carrying persisted cancellation and failure timestamps plus reasons through exact inbound-request detail and filtered inbound-request review, and browser integration coverage now exercises both the parked-request lifecycle and full request-status visibility. The provider-backed AI foundation is now live: `internal/ai` includes optional OpenAI configuration loading, the official OpenAI Go SDK, a Responses-API-backed provider adapter, and a coordinator flow that can claim one queued inbound request, execute a hard-capped tool loop with per-capability tool-policy enforcement, auto-run the first reporting read tool when policy allows, optionally route the result through one allowlisted specialist capability with a durable child run and delegation record, and persist the resulting run, step, artifact, and recommendation without making the default build and test flow depend on external credentials. `internal/app` now provides one shared backend seam for browser-session auth, bearer-session auth, queue processing, inbound-request submission, attachment download, operator review, approval decisions, proposal approval request, document review, accounting review, approval-queue review, proposal review, inventory review, work-order review, audit lookup, operations feed continuity, and the queue-oriented agent-chat entry path, plus server-rendered operator surfaces for sign-in, dashboard home, operations feed, dedicated request intake, dedicated agent chat, full inbound-request review, queue processing, request-detail inspection, approval actions, and downstream document, accounting, approval, proposal, inventory, work-order, and audit inspection. Exact `document_id` drill-down, dedicated `/app/review/documents/{document_id}`, `/app/review/approvals/{approval_id}`, `/app/review/proposals/{recommendation_id}`, `/app/review/accounting/{entry_id}`, `/app/review/accounting/control-accounts/{account_id}`, `/app/review/accounting/tax-summaries/{tax_code}`, `/app/review/inventory/{movement_id}`, `/app/review/inventory/items/{item_id}`, `/app/review/inventory/locations/{location_id}`, and `/app/review/audit/{event_id}` browser detail pages, exact work-order review filtering by both `document_id` and `work_order_id`, exact accounting journal drill-down by source `document_id` and exact `entry_id`, exact tax-summary drill-down by `tax_code`, cross-links between accounting summaries and their linked control accounts, exact inventory movement drill-down by `movement_id`, exact inventory item and location drill-down, exact audit-event drill-down by `event_id`, dedicated processed-proposal review with request-reference filtering and proposal-status summary, proposal-status summary cards that now jump directly into filtered browser review, exact proposal and approval drill-downs from the dashboard processed-proposal table, exact approval drill-downs from document-review rows when approval records already exist, proposal detail approval-request actions when the recommendation already identifies a submitted document, a dedicated approval-review page with queue and status filtering on top of the approval-queue seam, a dedicated inbound-request review page with status and exact `REQ-...` filtering on top of the inbound-request review seams, cross-links between proposals, approvals, inbound requests, documents, accounting, inventory movements, inventory items, inventory locations, work-order detail, and audit lookup, direct audit-page links back into exact inbound-request, approval, proposal, document, journal-entry, movement, item, location, and exact audit-event review, stronger inbound-request detail continuity into exact proposal and approval review plus request and recommendation audit, browser rendering of persisted AI step and delegation detail on inbound-request pages, accounting-browser tax-type plus control-account filtering on top of the shared review seams, exact inventory item and location continuity from stock rows, movement detail, and audit entities, filtered list continuity back from exact work-order detail into focused work-order review, dashboard inbound-request status summary cards that now jump directly into filtered request review, inbound-request dashboard and review rows that now continue into exact latest-run and exact proposal review, dashboard pending-approval rows that now continue into focused queue review, exact approval detail, and source-document audit, document and approval review surfaces that now continue back upstream into the originating request, exact proposal, and anchored AI run when that provenance exists, work-order list and detail review that now also continue back upstream into the originating request, exact proposal, exact approval, and anchored AI run when the linked source document carries that provenance, exact accounting detail plus exact inventory-movement and reconciliation review that now also continue back upstream into the originating request, exact proposal, exact approval, and anchored AI run when that provenance exists, and the new operations-feed plus agent-chat surfaces that continue directly into exact request, proposal, approval, and document review rather than inventing a second workflow truth. Exact inbound-request detail resolution by `run:<agent-run-id>`, `step:<agent-step-id>`, plus `delegation:<delegation-id>` lets `ai.agent_run`, `ai.agent_run_step`, and `ai.agent_delegation` audit entities continue directly into the precise request-level execution trail. That execution continuity now lands on anchored run, step, and delegation sections inside inbound-request detail, and the execution sections themselves now cross-link exact runs, steps, and delegations plus their audit trails, so browser review can follow one persisted AI execution trace instead of scanning disconnected blocks. `cmd/verify-agent`, `cmd/app`, and `cmd/set-password` continue to expose the live path through focused verification, the widened runnable application server, and password provisioning for guided testing.

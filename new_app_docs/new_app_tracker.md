...
Date: 2026-04-01
Status: Thin-v1 is complete through Milestone 9, and the active implementation phase is now v2 at Milestone 10; Slice 1 of the web rebuild is in progress in code through the new modular operator-entry template bundle, focused Slice 1 web verification plus repo build are passing, and workflow validation continues on the separate `docs/workflows/` track unless it promotes a bounded fix plan back into `new_app_docs/`
...
| Milestone 10 web rebuild planning | planned_with_preimplementation_slice_docs | `milestone_10_web_rebuild_plan.md` remains the preferred next browser-direction milestone, but it is now tightened so implementation must proceed through three large related slices with dedicated canonical slice plans written before implementation begins: Slice 1 architecture, shell, and operator-entry surfaces; Slice 2 review workbench family rebuild; Slice 3 detail continuity and closeout rebuild. Use `milestone_10_slice_1_architecture_and_operator_entry_plan.md`, `milestone_10_slice_2_review_workbench_plan.md`, and `milestone_10_slice_3_detail_and_closeout_plan.md` as the execution-control documents for Milestone 10 rather than treating the rebuild as an open-ended page-by-page effort. |
| Milestone 10 Slice 1 architecture and operator-entry implementation | in_progress | The web layer now has a modular embedded template bundle under `internal/app/web_templates` with a shared shell plus reusable flash, page-header, summary-card, detail-block, and empty-state partials. `/app`, `/app/login`, `/app/submit-inbound-request`, `/app/operations-feed`, and `/app/agent-chat` now render on the new bundle while the review and detail family still uses the legacy template path until Slice 2 and Slice 3. `go build ./cmd/... ./internal/...` and focused `internal/app` Slice 1 web tests are passing; full canonical DB-backed repo verification plus bounded manual browser review remain required before this slice can close. |
...
10. treat Milestone 10 Slice 1 as actively in progress in code through the new modular template bundle and the rebuilt operator-entry route family
11. finish the Slice 1 closeout work with canonical DB-backed repo verification plus bounded manual browser review on `/app/login`, `/app`, `/app/submit-inbound-request`, `/app/operations-feed`, and `/app/agent-chat`
12. then continue live workflow validation on the separate `docs/workflows/` track unless that browser review first promotes one bounded corrective slice
13. if later validation exposes product defects or missing support seams, add bounded fix plans back into `new_app_docs/` before implementation begins

...
Date: 2026-03-30
Status: Milestone 9 review completed cleanly on 2026-03-29; the bounded shared-backend correctness-hardening slice is now landed and verified, and the next session should first land one bounded test-harness hardening slice before resuming the paused live Phase 1 workflow validation
...
6. next implement one bounded test-harness hardening slice around disposable test-database advisory-lock behavior and stale-session diagnostics
7. verify that slice with the canonical `go build ./cmd/... ./internal/...` and `set -a; source .env; set +a; GOCACHE=/tmp/go-build go test -p 1 ./cmd/... ./internal/...` commands
8. only after that slice lands should the deferred live Phase 1 workflow validation resume on the real shared `/app` plus `/api/...` seam
...
4. next land one bounded test-harness hardening slice for advisory-lock acquisition behavior, stale-session visibility, and clearer blocked-test diagnostics in the disposable test database path
5. then resume real-browser workflow testing with Phase 1 workflow 2 draft save or edit -> queue -> process continuity
6. then execute Phase 1 workflow 3 processed proposal -> request approval -> approval decision continuity
7. record explicit live pass or fail evidence for the harness slice and both workflows in this tracker and `post_checkpoint_validation_and_user_testing_plan.md`
...
5. the next implementation step is therefore one bounded test-harness hardening slice around disposable test-database advisory-lock behavior and clearer blocked-test diagnostics
6. only after that slice lands should the remaining canonical live workflows resume in two phases: Phase 1 for draft-amend continuity plus approval-producing flow continuity, then Phase 2 for failed-provider or failed-processing visibility and final readiness closeout
6. once those workflows finish, decide explicitly whether the next session is:
7. one bounded post-checkpoint shared-backend or correctness slice, or
8. one explicitly promoted v2 work item under `new_app_docs/app_v2_plans/`
9. keep each newly promoted slice bounded to one concern rather than reopening completed milestone buckets
10. keep richer draft-attachment editing beyond the landed additive upload flow as residual only if later evidence proves it materially necessary
...
13. the correct next move is therefore one bounded shared-backend correctness slice and only then the deferred live workflows, not another implicit Milestone 9 extension or a silent scope drift into unrelated work

...
1. continue Milestone 6 from the now-landed provider-backed coordinator, shared backend seams, live verification command, request-submission contract, attachment transport contract, operator-review reads, and approval-decision contract by promoting browser-usable auth and session handling on the same seam
2. widen backend contracts in small slices so request submission, attachment transport, operator review, approval action, and later browser auth can sit on one web and mobile-ready foundation
3. after Milestone 6, implement the usable web application layer on the same backend foundation that a later v2 mobile client will reuse
...
1. extend the now-landed API contract set on top of the OpenAI-backed coordinator, shared backend seams, and live verification command with browser-usable auth and session slices after the landed request-submission, attachment-transport, operator-review, and approval-decision contracts
2. widen that backend contract set in small slices without creating a second truth owner
...
5. shared backend contracts, a focused live-provider verification command, queued-request processing, request submission, attachment transport, operator review, and approval action now exist for driving the live path outside direct service calls, so the next high-value Milestone 6 gap is browser-usable auth and session promotion on top of that seam

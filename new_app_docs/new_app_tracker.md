...
Date: 2026-03-30
Status: Milestone 9 review completed cleanly on 2026-03-29; the remaining live workflow validation is now deliberately paused while one bounded shared-backend correctness-hardening slice lands first
...
5. next implement one bounded shared-backend correctness-hardening slice before any live browser workflow testing resumes
6. required outcomes for that slice are request-to-message ownership validation on draft updates, persistence of existing-draft metadata edits, atomic draft-save composition across request or message or attachment work, and HTTPS-oriented `Secure` hardening for browser-session cookies
7. verify that slice with `go build ./cmd/... ./internal/...` plus `set -a; source .env; set +a; GOCACHE=/tmp/go-build go test -p 1 ./cmd/... ./internal/...`
8. only after that slice lands should the deferred live Phase 1 then Phase 2 workflow validation resume later from the same canonical plan
...
3. next implement the bounded shared-backend correctness-hardening slice rather than restarting real-browser workflow testing in the same session
4. add or update focused regression coverage for draft-update ownership validation, existing-draft metadata persistence, atomic draft-save behavior, and browser-session cookie security flags
5. run `go build ./cmd/... ./internal/...` and `set -a; source .env; set +a; GOCACHE=/tmp/go-build go test -p 1 ./cmd/... ./internal/...`
6. carry the remaining live Phase 1 then Phase 2 workflow validation forward as deferred work in `post_checkpoint_validation_and_user_testing_plan.md` rather than mixing it into the same implementation slice
7. update `docs/workflows/application_workflow_catalog.md` and `docs/workflows/end_to_end_validation_checklist.md` later only if the deferred live workflow validation materially changes the durable workflow reference or reusable workflow checklist
...
4. implement one bounded shared-backend correctness-hardening slice first: draft-save ownership validation, existing-draft metadata persistence, atomic draft-save composition, and browser-session cookie security hardening
5. only after that slice lands cleanly, resume the remaining canonical live workflows in two phases later: Phase 1 for draft-amend continuity plus approval-producing flow continuity, then Phase 2 for failed-provider or failed-processing visibility and final readiness closeout

...
3. implement `milestone_16_structural_and_ai_capability_plan.md` next as a full AI-layer and foundational workflow-execution overhaul: start from the planned non-urgent refactors, accounting-impact triage, safe no-accounting behavior for non-accounting events, accounting proposal generation from inbound requests, multi-event decomposition, specialist execution, recovery loops, and capability expansion, but make the first externally meaningful checkpoint the request-to-accounting-entry workflow rather than waiting for every structural cleanup slice or future non-accounting persistence model to land

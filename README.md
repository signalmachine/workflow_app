...
The v2 planning documents in [`new_app_docs/`](./new_app_docs) remain the canonical source for active scope, architecture guardrails, sequencing, status, and next steps. Archived milestone detail now lives under `new_app_docs/v2_archive/` and should be used only when the current task needs that historical context.
...
Run the canonical browser-closeout seed when the served app must validate exact request -> proposal -> approval -> document -> accounting continuity on the same backend:
...
1. `cmd/verify-agent` prefers `TEST_DATABASE_URL` over `DATABASE_URL` unless `-database-url` is passed explicitly
2. when browser proof is running against the served app, use `-database-url "$DATABASE_URL"` so the seed records land on the same backend the browser is reading
3. the command now prints the verification org slug, browser credentials, request reference, and continuity ids needed for Playwright or manual browser proof
...
When the served app is under browser review, rebuild after frontend changes and restart `cmd/app` before diagnosing stale UI failures. `cmd/app` serves the embedded files from `internal/app/web_dist`, not the source files in `web/`.
...
Run the real-browser Playwright closeout suite:

```bash
npm --prefix web run test:e2e -- milestone13_closeout.spec.ts
```

For workflow-critical browser closeout, prefer stable route assertions based on headings, primary actions, exact ids, and review-link continuity over brittle copy-only markers.
...
http://127.0.0.1:8080/app/admin/master-data
http://127.0.0.1:8080/app/admin/lists

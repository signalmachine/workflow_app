...
3. resume the remaining canonical live workflows for draft-amend continuity, approval-producing flow continuity, and failed-provider or failed-processing visibility now that the first provider-quality blocker and the processed-proposal approval-request seam blocker are cleared
4. keep the remaining validation work bounded to those canonical workflows and any narrow correctness fixes they expose
5. end the overall validation slice with one explicit readiness result for supervised AI-backed user testing or with an explicit blocker list if readiness is not yet achieved

Planned next-session review and testing order:

1. start with a focused code review of the live `/app` plus `/api/...` proposal, approval, draft-amend, and failure-visibility seams before making any further code changes
2. rerun `set -a; source .env; set +a; go run ./cmd/verify-agent` to confirm the live provider seam still passes after the latest shared-backend hardening
3. run `set -a; source .env; set +a; APP_LISTEN_ADDR=127.0.0.1:18080 go run ./cmd/app`
4. execute the remaining canonical live workflows in order: draft save or edit -> queue -> process, processed proposal -> request approval -> approval decision, and failed provider or failed processing visibility
...
3. the first blocker-remediation slice is now complete, so resume the remaining canonical live workflows for draft-amend continuity, approval-producing flow continuity, and failed-provider or failed-processing visibility
4. once those workflows finish, decide explicitly whether the next session is:

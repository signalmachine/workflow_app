...
19. the post-closeout codebase review then found one active product gap and two active documentation gaps that became the first Milestone 14 work rather than silent drift:
20. the backend already supported parked-request draft update, queue, cancel, amend, and delete actions, but the promoted Svelte runtime exposed only new-request creation plus read-only request detail continuity before the Milestone 14 Slice 1 correction
21. the inbound-request user guide pointed at a nonexistent browser path for queue processing before the Milestone 14 documentation-truth correction
22. route and workflow docs mixed `/app/inbound-requests` and `/app/review/inbound-requests` before the Milestone 14 route-contract correction
23. the shared desktop shell needed one bounded layout correction before the Milestone 14 shell-layout checkpoint
24. the promoted accounting area lacked baseline operator financial statements before the Milestone 14 accounting-report checkpoint
...
47. Milestone 16 must also make accounting impact explicit: no-accounting-impact business events should return a clear operator comment without creating accounting records, future supported non-accounting event persistence must use explicit non-accounting tables and backend services rather than accounting placeholders, unsupported non-accounting events must not invent persistence paths, and requests containing multiple accounting events such as multiple vendor invoices should decompose into separate proposal/document candidates rather than one collapsed accounting entry
...
3. then use Milestone 16 for the remaining non-urgent structural, operational, and AI capability improvements that were validated but are not urgent defects, including the newly explicit accounting impact, safe no-accounting behavior, accounting proposal generation, and multi-event decomposition slices, make the first externally meaningful Milestone 16 checkpoint the request-to-accounting-entry workflow, defer non-accounting event persistence until explicit event types and tables are designed, and keep the milestone open until the accounting path is implemented and tested end to end

...
Date: 2026-04-03
Status: Thin-v1 is complete through Milestone 9, the active implementation phase is now an ambitious best-practice v2 phase across the implemented Milestone 10 and Milestone 11 browser history, the implemented Milestone 12 admin-maintenance program, and the now-started Milestone 13 Svelte web migration, the rebuilt modular web bundle still covers the operator-entry surfaces, the promoted review workbench family, the promoted detail-route family, the grouped landing pages for operations, review, and inventory, the searchable route catalog plus utility surfaces, and the new role-aware operator home, `web/` now also contains the landed SvelteKit Slice 1 scaffold with shared shell, login, protected-route posture, and direct build output into `internal/app/web_dist`, `/app/settings` stays explicitly user-scoped while `/app/admin` becomes a grouped privileged maintenance hub with bounded accounting, party, access, and inventory maintenance seams on the shared backend, the approved next browser architecture is now the Svelte-based web replacement on the same Go backend and auth foundation, active implementation posture now explicitly allows stronger capability, stronger architecture, and proactive refactor or rebuild work when justified by established best practices, and Milestone 10 browser-review plus workflow-continuity evidence still needs to catch up on the separate `docs/workflows/` track
...
| Milestone 13 Svelte web migration | slice_1_implemented_in_code | `milestone_13_svelte_web_migration_plan.md` is now active in implementation. Slice 1 is landed in code through the new `web/` SvelteKit scaffold, protected app-route and public login-route structure, shared shell and design-token foundation, minimal API and session bootstrap layer, frontend build and test tooling, direct build output into `internal/app/web_dist`, and embedded Go static serving behind `WORKFLOW_WEB_FRONTEND=svelte` so the new shell can be exercised under `/app` without forcing cutover yet. `npm --prefix web run check`, `npm --prefix web run test`, `npm --prefix web run build`, focused `internal/app` SPA-serving tests, `go build ./cmd/... ./internal/...`, and `gopls` diagnostics all passed. Slice 2 workflow-surface migration is now the next browser implementation step. |
...
13. the next active browser implementation milestone remains Milestone 13, and it should be treated as the canonical Svelte migration program rather than as another extension of the old Go-template browser stack
14. Milestone 13 Slice 1 foundation and shell work is now implemented in code, so the next promoted browser implementation work is Slice 2 workflow-surface migration rather than additional old-stack browser cleanup
...
5. keep the separate Milestone 10 closeout sweep on the workflow track, but treat it as parity and validation input to Milestone 13 rather than as a blocker on continuing from the now-landed Svelte foundation work
...
9. the next planned browser implementation gap is now Milestone 13 Slice 2 workflow-surface migration on top of the landed Slice 1 scaffold, while later privileged-maintenance follow-on work remains bounded shared-backend product work rather than old-stack browser work

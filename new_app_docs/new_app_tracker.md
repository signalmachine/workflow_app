...
Date: 2026-03-30
Status: Milestone 9 review completed cleanly on 2026-03-29; the bounded shared-backend correctness-hardening slice, the bounded test-harness advisory-lock hardening slice, the bounded web visual-refresh implementation slice, its bounded corrective follow-up slice, and the bounded operations-feed plus agent-chat browser-surface slice are now landed, automated verification is complete, bounded manual browser-review evidence for the refreshed and newly added browser pages is still pending, and workflow validation continues on the separate `docs/workflows/` track
...
| Web UI streamlining plan | planned | `web_ui_streamlining_plan.md` captures the next bounded browser-usability cleanup slice. The current web layer is usable but still feels visually noisy and too dense on the dashboard, review pages, and detail pages. This plan records the future work as a presentation-focused follow-on so the underlying workflow behavior stays unchanged. |
...
11. the next active implementation step is one bounded performance pass on the `internal/app` test suite to identify the heaviest paths and land only low-risk speedups if they are clearly worthwhile
...
7. next take one bounded performance pass on the `internal/app` test suite focused on identifying the slowest paths and applying only narrow, low-risk speedups that do not weaken workflow-critical coverage
8. after that bounded performance pass, keep live workflow validation on the separate `docs/workflows/` track
...
7. `operator_communication_and_intake_surfaces_plan.md` is now landed in full, so the next promoted implementation work is the bounded `internal/app` test-suite performance pass
8. after that bounded performance pass, keep workflow validation on the separate `docs/workflows/` track and then decide explicitly whether any later work is a new bounded correctness slice or an explicit v2 promotion
9. once that bounded test-performance pass and the later workflow validation pass have both progressed, decide explicitly whether the next session is:
...
16. the next promoted implementation work is one bounded `internal/app` test-suite performance pass so long as it stays focused on measurement plus low-risk speedups rather than weakening the suite's workflow-critical coverage

...
| Milestone 7 usable web application layer | in_progress | the `/app` browser surface now covers both the first operator loop and the next downstream review slice on the same shared backend seam: operators can sign in with browser sessions, submit inbound requests with file attachments, process the next queued request, review recent requests and pending approvals, open inbound-request detail, inspect attachments plus AI runs, artifacts, recommendations, and proposals, decide approvals, and now continue into document review plus accounting review for journal entries, control-account balances, and tax summaries without dropping back to scripts or raw API exploration. The shared backend seam now also exposes cookie-auth-capable review endpoints for `GET /api/review/documents`, `GET /api/review/accounting/journal-entries`, `GET /api/review/accounting/control-account-balances`, and `GET /api/review/accounting/tax-summaries`; remaining Milestone 7 work is to keep widening this browser layer across the remaining reporting surfaces and operator continuity without creating a second backend; see `web_application_layer_plan.md` |
...
1. continue Milestone 7 from the landed `/app` browser slices by widening the same web layer into the remaining inventory, work-order, and audit review surfaces without splitting backend ownership
...
1. widen the landed `/app` browser slices so operators can move from request intake, approval work, and financial-document review into the remaining broader reporting surfaces on the same backend contracts

...
4. `pending_live_validation`: implemented, but still awaiting explicit live workflow validation; after the Milestone 14 final confidence gate passed, remaining deeper live-validation backlog items are intentionally deferred to the user-testing period unless a new concrete blocker appears

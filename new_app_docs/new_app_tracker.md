...
Date: 2026-03-30
Status: Milestone 9 review completed cleanly on 2026-03-29; the bounded shared-backend correctness-hardening slice and the bounded test-harness advisory-lock hardening slice are now landed and verified, and the next session should land the bounded web visual-refresh slice before the paused live Phase 1 workflow validation resumes
...
| Post-checkpoint validation Step 2-5 browser and workflow checks | paused | On 2026-03-28 the real browser and API seam was exercised against the configured development database and live provider path: `POST /api/session/login`, `GET /api/session`, `GET /app`, `POST /api/inbound-requests`, `POST /api/agent/process-next-queued-inbound-request`, `GET /api/review/inbound-requests`, `GET /api/review/inbound-request-status-summary`, `GET /api/review/inbound-requests/REQ-000001`, `GET /api/review/processed-proposals`, `GET /api/review/approval-queue`, and `GET /app/inbound-requests/REQ-000001` all returned success responses with the expected persisted request, run, step, artifact, and proposal continuity. The first live blocker from that pass is now cleared: `internal/ai` was hardened so request messages and derived texts are primary evidence, queue-summary context is explicitly secondary, obviously stale `processing`-style briefs are rejected, and the OpenAI provider now gets one bounded repair turn before failing when the first structured brief stays too generic. On 2026-03-28 `go build ./...`, `set -a; source .env; set +a; timeout 600s go test -p 1 ./...`, and `set -a; source .env; set +a; go run ./cmd/verify-agent` all passed after that fix, with the live verification summary returning a request-specific warehouse-pump inspection brief instead of stale queue-status wording. A second bounded shared-backend correctness slice is now also landed inside the same validation phase: processed-proposal review now derives pre-approval document and suggested-queue continuity from recommendation payload when no approval link exists yet, and the shared `/api/...` plus `/app/...` seams now support requesting workflow approval directly from a processed proposal with atomic recommendation-link persistence. `go build ./...`, `set -a; source .env; set +a; GOCACHE=/tmp/go-build go test -p 1 ./...`, and targeted web coverage all passed after that change. A third bounded correctness slice is now also landed on 2026-03-29 for the still-pending failed-processing workflow: when queued-request processing fails after claim, both `POST /api/agent/process-next-queued-inbound-request` and `/app/agent/process-next-queued-inbound-request` now preserve exact `REQ-...` troubleshooting continuity instead of dropping operators onto a generic failure state, and new `internal/app` integration coverage verifies that the API failure response carries the exact request reference while the browser failure redirect lands on the failed request detail with failed run and step visibility. `go build ./cmd/... ./internal/...`, focused `internal/app` DB-backed regression checks, and `set -a; source .env; set +a; GOCACHE=/tmp/go-build go test -p 1 ./cmd/... ./internal/...` all passed after that hardening. A fourth bounded repo-verification slice is now also landed on 2026-03-29 for the planned Phase 1 foundational workflows: new `internal/app` integration coverage now exercises the full draft save and edit -> queue -> process -> exact request and proposal continuity chain plus the full processed proposal -> request approval -> approval decision -> exact approval and document continuity chain across both `/api/...` and `/app/...` review seams. `go build ./cmd/... ./internal/...`, focused `internal/app` DB-backed continuity checks, and `set -a; source .env; set +a; GOCACHE=/tmp/go-build go test -count=1 -p 1 ./cmd/... ./internal/...` all passed after that addition. On 2026-03-30 the bounded shared-backend correctness slice identified by the full end-to-end review also landed cleanly: draft-save now rejects mismatched `request_id` plus `message_id` combinations, existing draft saves now persist updated request metadata, draft-save composition is now atomic across request or message plus attachment work, and browser-session cookies now set `Secure` on HTTPS requests without weakening local HTTP development. Focused `internal/app` regression coverage was added for those seams, `go build ./cmd/... ./internal/...` passed, `set -a; source .env; set +a; GOCACHE=/tmp/go-build go test -v ./internal/intake -count=1` passed after clearing stale test-held migration locks in the disposable test database, and the canonical `set -a; source .env; set +a; GOCACHE=/tmp/go-build go test -p 1 ./cmd/... ./internal/...` command then completed cleanly. That same verification pass exposed one bounded harness issue worth hardening before the next live workflow pass, and that harness issue is now also closed on 2026-03-30: the shared DB-backed test harness now uses bounded advisory-lock acquisition with holder-session diagnostics instead of an unbounded wait, the migration integration harness reuses the same path, and focused integration coverage verifies that blocked lock acquisition surfaces the holder session in the timeout error. A final bounded local-setup hardening slice is now also landed on 2026-03-30: runnable commands auto-load `.env` without overriding explicitly exported variables, `cmd/bootstrap-admin` now provides an idempotent friendly main-database org plus admin bootstrap path for browser sign-in, and `docs/user_guides/` now includes focused run and sign-in guidance that keeps `DATABASE_URL` and `TEST_DATABASE_URL` responsibilities explicit. `go build ./cmd/... ./internal/...`, the canonical `set -a; source .env; set +a; GOCACHE=/tmp/go-build go test -p 1 ./cmd/... ./internal/...`, and direct `go run ./cmd/app` startup all passed after that setup hardening. One additional bounded pre-validation slice is now also explicitly planned before live workflow review resumes: refresh the web visual layer so the browser-led validation and operator testing run against a more credible enterprise-style light UI. See `web_visual_refresh_plan.md` and `post_checkpoint_validation_and_user_testing_plan.md`. |
| Milestone 9 user-testing readiness hardening | done | Milestone 9 is now complete in `milestone_9_user_testing_readiness_hardening_plan.md`, and the explicit implementation-versus-plan review was also completed cleanly on 2026-03-29. Slice 1 auth hardening is complete: browser-session and bearer-session issuance now require a password-backed credential check on the shared `identityaccess.users` record while preserving one session foundation for the browser and the later mobile client on the same `/api/...` seam. Slice 2 bounded AI capability expansion is complete: the OpenAI-backed coordinator keeps the same bounded coordinator-plus-specialist architecture but now exposes request-scoped read-only tools for current inbound-request detail and current processed-proposal continuity, so provider execution can gather more request-relevant review context than queue summary alone without widening into write-capable autonomy or weakening tool-policy enforcement. Slice 3 shared web or API seam decomposition is complete: `internal/app` is split by seam into dedicated API session, inbound, review, and approval handler files plus dedicated web session or inbound and review-surface files, reducing the largest maintenance-risk concentration without changing the shared HTTP contracts or the thin-v1 web stack. Closeout verification passed on 2026-03-29: `go build ./cmd/... ./internal/...`, `set -a; source .env; set +a; GOCACHE=/tmp/go-build go test -p 1 ./cmd/... ./internal/...`, and `set -a; source .env; set +a; go run ./cmd/verify-agent` all completed cleanly. The required post-closeout review then found no material drift across the auth, AI, or seam-decomposition slices, and a fresh `set -a; source .env; set +a; go run ./cmd/verify-agent` rerun also completed cleanly on 2026-03-29 with `REQ-000001` processed through a completed coordinator run and a request-specific urgent warehouse-pump recommendation summary, so the repository should now continue the paused post-checkpoint live validation slice rather than reopen Milestone 9 hardening. Anything under `examples/` remains read-only reference material and is not part of the active `workflow_app` codebase. The historical accounting-agent proof-of-concept remains an external comparison point at https://github.com/signalmachine/accounting-agent-app rather than an in-repo reference tree. |
...
9. the deferred live Phase 1 workflow validation should resume immediately after that refresh lands on the real shared `/app` plus `/api/...` seam
...
6. then resume real-browser workflow testing with Phase 1 workflow 2 draft save or edit -> queue -> process continuity
7. then execute Phase 1 workflow 3 processed proposal -> request approval -> approval decision continuity
8. record explicit live pass or fail evidence for those workflows in this tracker and `post_checkpoint_validation_and_user_testing_plan.md`
...
6. the next implementation step is the bounded web visual-refresh slice in `web_visual_refresh_plan.md`
7. after that slice lands, resume the remaining canonical live workflows in two phases: Phase 1 for draft-amend continuity plus approval-producing flow continuity, then Phase 2 for failed-provider or failed-processing visibility and final readiness closeout
8. once those workflows finish, decide explicitly whether the next session is:
9. one bounded post-checkpoint shared-backend or correctness slice, or
10. one explicitly promoted v2 work item under `new_app_docs/app_v2_plans/`
11. keep each newly promoted slice bounded to one concern rather than reopening completed milestone buckets
12. keep richer draft-attachment editing beyond the landed additive upload flow as residual only if later evidence proves it materially necessary
...
14. the remaining underpowered browser presentation is a worthwhile bounded pre-validation concern because the next planned work is explicitly browser-led
15. the correct next move is therefore the bounded visual-refresh slice first and then the deferred live workflows, rather than another implicit Milestone 9 extension or a silent scope drift into unrelated work

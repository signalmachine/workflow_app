...
27. the latest continuity slice now promotes inventory item and location review into dedicated browser detail stops through `/app/review/inventory/items/{item_id}` and `/app/review/inventory/locations/{location_id}` on top of the already-landed stock and movement seams, so stock rows, movement detail, and audit entities can continue into one exact item or location page with stock and movement context instead of only anchoring into the broader inventory list
28. the latest continuity slice now turns accounting control-account balances and tax summaries into dedicated browser review stops through `/app/review/accounting/control-accounts/{account_id}` and `/app/review/accounting/tax-summaries/{tax_code}`, and the shared accounting-review seam now also supports exact `tax_code` filtering so browser and API callers can land on one tax summary directly instead of treating those sections as summary-only tables
29. the latest continuity slice now extends work-order review with exact `work_order_id` filtering through both `/api/review/work-orders?work_order_id=...` and `/app/review/work-orders?work_order_id=...`, and it turns `/app/review/work-orders/{work_order_id}` into a stronger browser stop by linking back into focused list review plus direct accounting review on the same shared seam instead of leaving work-order detail as a dead end
30. the latest continuity slice now tightens row-level proposal and approval drill-downs by turning proposal-status summary cards into filtered navigation, adding exact proposal and approval links to the dashboard processed-proposal table, and linking document-review approval cells directly into exact approval detail where the approval record already exists instead of making operators reopen broader filtered queues
31. the latest continuity slice now turns `/app/review/inventory/{movement_id}` into a stronger browser stop by adding focused item-history, stock-balance, item-reconciliation, source-location, destination-location, document-reconciliation, and accounting-review links on top of the already-landed movement detail and reconciliation seams, so exact movement review continues cleanly into adjacent stock, document, execution, and posting context instead of stalling on an isolated detail page
32. the latest continuity slice now extends exact inbound-request detail lookup to resolve `run:<agent-run-id>` and `delegation:<delegation-id>` on top of the existing `/app/inbound-requests/{request_reference_or_id}` and `/api/review/inbound-requests/{request_reference_or_id}` seams, and audit-page entity links for `ai.agent_run` plus `ai.agent_delegation` now continue back into the exact inbound-request execution trail instead of dead-ending on generic audit results
33. the latest continuity slice now anchors AI run and delegation blocks inside inbound-request detail itself, so those exact audit-driven execution links now land the operator on the precise persisted run or delegation record instead of only the broader request page
34. the latest continuity slice now links inbound-request runs, steps, and delegations together on-page with exact anchors plus run and delegation audit links, so operators can traverse one execution trace in the browser instead of scanning disconnected sections manually
35. the latest continuity slice now extends exact inbound-request detail lookup to resolve `step:<agent-step-id>` as well, and audit-page entity links for `ai.agent_run_step` or `ai.agent_step` plus step-section audit links now continue back into the exact inbound-request step block instead of stopping at the broader request page
36. the latest continuity slice now turns dashboard inbound-request status summary cards into direct filtered-request review links and adds exact latest-run plus exact proposal continuations from inbound-request dashboard and review rows, so operator AI-status review can move straight from queue health into the precise execution or proposal surface instead of stalling at passive status text
...
38. the remaining Milestone 7 work is now operator continuity, richer drill-downs, and focused refinement on top of those landed review surfaces without creating a second backend or reviving broad manual-entry UI scope
